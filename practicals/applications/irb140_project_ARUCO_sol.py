...
def pick_aruco(robot, gripper, frame, tp):
    ...
    gripper.open()
    robot.moveL(target_position=T.pos()+np.array([0, 0, +0.1]), target_orientation=T.R(), vmax=0.2, precision=True)
    robot.moveL(target_position=T.pos(), target_orientation=T.R(), precision=True, vmax=0.02)
    gripper.close()
