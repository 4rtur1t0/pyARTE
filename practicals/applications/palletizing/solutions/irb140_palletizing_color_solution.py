...
def pick(robot, gripper):
    ...
    """
    tp1 = Vector([0.6, 0.04, 0.25])  # approximation
    tp2 = Vector([0.6, 0.04, 0.19])  # pick
    to = Euler([0, np.pi, 0])
    gripper.open()
    robot.moveJ(target_position=tp1, target_orientation=to, precision=True, speed_factor=1.0)
    robot.moveL(target_position=tp2, target_orientation=to, precision=True, speed_factor=0.5)
    gripper.close()
