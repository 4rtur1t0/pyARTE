...
def pick_aruco(robot, gripper, frame, tp):
    ...
    gripper.open()
    robot.moveL(target_position=T.pos()+np.array([0, 0, +0.1]), target_orientation=T.R(), speed_factor=3.0, precision=False)
    robot.moveL(target_position=T.pos(), target_orientation=T.R(), precision=True, speed_factor=1.5)
    gripper.close()
