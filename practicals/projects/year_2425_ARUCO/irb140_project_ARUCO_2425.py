...
def pick_and_place_arucos():
    ...
    id, Tca = camera.detect_closer_aruco(show=False, aruco_size=0.03)
    # La transformación total es:
    T0a = T0v*Tvc*Tca
    # ¿es alcanzable la posición/orientación T0a? ... obsérvela en Coppelia
    frame.show_target_point(target_position=T0a.pos(), target_orientation=T0a.R())
    robot.moveJ(target_position=T0a.pos(), target_orientation=T0a.R())
