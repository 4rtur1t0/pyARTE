...
class ReferenceFrame(CoppeliaObject):
    ...
    def show_target_point(self, target_position, target_orientation):
        T = HomogeneousMatrix(target_position, target_orientation)
        self.set_position_and_orientation(T)

    def show_target_points(self, target_positions, target_orientations):
...
class ConveyorBelt(CoppeliaObject):
    def __init__(self, simulation):
        CoppeliaObject.__init__(self, simulation=simulation)
        self.script_handle = None

    def start(self, name='/conveyor'):
        # Get the handles of the relevant objects
        handle = self.simulation.sim.getObject(name)
        self.handle = handle
        self.script_handle = self.simulation.sim.getScript(self.simulation.sim.scripttype_childscript, handle)

    def force_start(self):
        new_value = 1
        result = self.simulation.sim.callScriptFunction('updateMyVariable', self.script_handle, new_value)
